    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            try:
                ...
                proc.kill()
            proc.communicate()
            last_error = TimeoutError(f"judge command timed out after {timeout} seconds")
        else:
            if proc.returncode != 0:
                ...
                try:
                    ...
                    result.backend_meta["command_attempt"] = attempt + 1
                    result.backend_meta["command_retries"] = retries
                    return result
                except json.JSONDecodeError as exc:
                    last_error = RuntimeError(f"judge command returned non-JSON stdout: {stdout[:500]}")
        if attempt < retries:
...
def score_image(
    ...
    if backend == "mock":
        ...
        result = command_score(image, anchors, judge_command, timeout, quality_config, retries=command_retries)
    else:
    ...
    for view in views:
        ...
        try:
            ...
            view_scores[viewport] = score_image(
                ...
                quality_config=quality_config,
                command_retries=getattr(args, "judge_retries", 0),
            )
