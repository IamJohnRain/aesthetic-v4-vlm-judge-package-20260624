...
try:
    import json5
except ImportError:  # pragma: no cover - optional tolerance dependency
    json5 = None
...
def loads_relaxed_json(text: str) -> dict[str, Any]:
    try:
...
def extract_json_object(text: str) -> dict[str, Any]:
    ...
    try:
        return loads_relaxed_json(stripped)
    except Exception:
        pass
    ...
    if start >= 0 and end > start:
        try:
            return loads_relaxed_json(stripped[start : end + 1])
        except Exception as exc:
            raise ValueError(f"model output contained malformed JSON: {exc}; raw={text[:500]}") from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--parse-fallback-score-only",
        action=argparse.BooleanOptionalAction,
        default=os.environ.get("PANGU_JUDGE_PARSE_FALLBACK_SCORE_ONLY", "1") != "0",
        help="When full JSON parsing keeps failing, retry once with score-only output.",
    )
...
def main() -> int:
    ...
    try:
        ...
        if payload is None:
            if args.output_mode != "score-only" and args.parse_fallback_score_only:
                original_mode = args.output_mode
                args.output_mode = "score-only"
                try:
                    raw, api_retry_count = call_model(args, request, token, parse_retry=True)
                    retry_count += api_retry_count + 1
                    choices = raw.get("choices") or []
                    if not choices:
                        raise RuntimeError(f"API response has no choices: {json.dumps(raw, ensure_ascii=False)[:1000]}")
                    message = choices[0].get("message") or {}
                    payload = extract_json_object(content_text(message.get("content")))
                except Exception as fallback_exc:
                    raise RuntimeError(
                        f"{last_parse_error or 'model output did not contain JSON'}; "
                        f"score-only fallback also failed: {fallback_exc}"
                    ) from fallback_exc
                finally:
                    args.output_mode = original_mode
                normalized_output_mode = "score-only"
            else:
                raise RuntimeError(str(last_parse_error or "model output did not contain JSON"))
        else:
            normalized_output_mode = args.output_mode
        ...
        if normalized_output_mode != args.output_mode:
            normalized.setdefault("backend_meta", {})["fallback_from_output_mode"] = args.output_mode
            normalized["backend_meta"]["fallback_reason"] = "full_json_parse_failed"
    except Exception as exc:
